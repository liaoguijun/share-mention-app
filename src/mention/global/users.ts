import type { ApiChat, ApiUser, ApiUserStatus } from '../types';
...
export function getUserStatus(
  lang: any, user: ApiUser, userStatus: ApiUserStatus | undefined, serverTimeOffset: number,
) {
...
export function filterUsersByName(
  ...
  return userIds.filter((id) => {
    ...
    return (name && searchWords(name)) || Boolean(user.usernames?.find(({ username }:any) => searchWords(username)));
  });
}
...
export function getMainUsername(userOrChat: ApiUser | ApiChat) {
  return userOrChat.usernames?.find((u:any) => u.isActive)?.username;
}
