import { AnyToVoidFunction } from '../global'
...
export function onIdle(cb: any, timeout?: number) {
  // eslint-disable-next-line no-restricted-globals
...
let fastRafCallbacks: any[] | undefined;
let fastRafPrimaryCallbacks: any[] | undefined;

// May result in an immediate execution if called from another `requestAnimationFrame` callback
export function fastRaf(callback: any, isPrimary = false) {
  if (!fastRafCallbacks) {
...
export function fastRafPrimary(callback: any) {
  fastRaf(callback, true);
}

let onTickEndCallbacks: any[] | undefined;
let onTickEndPrimaryCallbacks: any[] | undefined;

export function onTickEnd(callback: any, isPrimary = false) {
...
export function onTickEndPrimary(callback: any) {
  onTickEnd(callback, true);
}

let beforeUnloadCallbacks: any[] | undefined;

export function onBeforeUnload(callback: any, isLast = false) {
